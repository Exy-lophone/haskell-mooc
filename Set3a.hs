...
palindrome str
    | length str <= 1 = True
    | head str == last str = palindrome $ init . tail $ str  
    | otherwise = False
...
whileRight check x = let either = check x in
    case either of 
...
joinToLength len strings = [x | a <- strings, b <- strings, let x = a++b, length x == len]
...
(+|+) [] (yfirst:ys) = [yfirst] 
(+|+) [] [] = [] 
(+|+) (xfirst:xs) (yfirst:ys) = [xfirst,yfirst]
...
sumRights (x:xs) =
    case x of
        (Right value) -> value + sumRights xs
        (Left value) -> sumRights xs
...
interpreter' xPos yPos (x:xs) = 
    case x of
        "up" -> interpreter' xPos (yPos+1) xs
        "down" -> interpreter' xPos (yPos-1) xs
        "left" -> interpreter' (xPos-1) yPos xs
        "right" -> interpreter' (xPos+1) yPos xs
        "printY" -> show yPos : interpreter' xPos yPos xs
        "printX" -> show xPos : interpreter' xPos yPos xs
interpreter' xPos yPos [] = []
